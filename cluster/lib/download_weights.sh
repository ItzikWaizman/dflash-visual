...
#
# Recommended: submit as an sbatch job (CPU-only, no GPU needed):
#   mkdir -p /scratch300/$USER/dflash_vlm/dflash-visual/output_logs
#   sbatch -A gpu-tad-wolf_v2 -p gpu-tad-pool --qos=owner \
#          --time=02:00:00 --cpus-per-task=2 --mem=8G \
#          -o output_logs/download_weights.out \
#          --job-name=dflash_download \
#          --chdir /scratch300/$USER/dflash_vlm/dflash-visual/ \
#          ./cluster/lib/download_weights.sh
#
# Pass any extra flags through to the Python script, e.g.:
